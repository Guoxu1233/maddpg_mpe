...
class MultiAgentEnv(gym.Env):
    ...
    }#human是用来实时的打印在屏幕上的，rgb_array是用来保存的
    ...
    def render(self, mode='human', close=False):
        if close:#关闭现有的所有渲染器
            ...
            for viewer in self.viewers:
                if viewer is not None:
                    viewer.close()
            self.viewers = [None] * len(self.viewers)
            return []
        ...
        for i in range(len(self.viewers)):#创造渲染器
        ...
        if self.render_geoms is None:
                ...
                if 'agent' in entity.name:
                    ...
                    if not entity.silent:
                        for ci in range(self.world.dim_c):
                            comm = rendering.make_circle(entity.size / self.world.dim_c)
                            comm.set_color(1, 1, 1)
                            comm.add_attr(xform)
                            offset = rendering.Transform()
                            comm_size = entity.size / self.world.dim_c
                            offset.set_translation(ci * comm_size * 2 - entity.size + comm_size, 0)
                            comm.add_attr(offset)
                            self.comm_geoms.append(comm)
                else:
            ...
            for wall in self.world.walls:
                ...
                geom.set_color(*wall.color, alpha=0.5 if not wall.hard else 1.0)
                self.render_geoms.append(geom)
            ...
            for viewer in self.viewers:
                ...
                for comm_geom in self.comm_geoms:
                    viewer.add_geom(comm_geom)
        ...
        if mode == 'rgb_array':
            return results
        return results if mode == 'human' else results[0]
